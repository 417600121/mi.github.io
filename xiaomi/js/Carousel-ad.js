...
/*设置每一页的列表宽度 */
var ad_right=document.getElementsByClassName("ad-right");

var two="531px"
var three="796px"
var four="993px"
ad_right[0].style.width=four;
ad_right[1].style.width=four;
ad_right[2].style.width=three;
ad_right[3].style.width=four;
ad_right[4].style.width=three;
ad_right[5].style.width=three;
ad_right[6].style.width=two;
ad_right[7].style.width=three;
ad_right[8].style.width=three;
ad_right[9].style.width=three;

//========================================
// 开始设置轮播广告
var bigad=document.getElementById('bigad');
var points=document.getElementsByClassName('ad-point');
var adpoints=document.getElementById('ad-points');
var ad_link=document.getElementById("ad-link");
var index=0;//表示当前广告页
var times=0;//定时器次数，方便控制延时
var ad_img_src=[
	["./img/ad-list-1.png","https://www.mi.com/buy/detail?product_id=14439"],
	["./img/ad-list-2.png","https://www.mi.com/buy/detail?product_id=10000296"],
	["./img/ad-list-3.png","https://www.mi.com/a/h/13951.html"],
	["./img/ad-list-4.png","https://www.mi.com/a/h/20885.html?sign=0974c4be066ef79da5a915097858def9"]
]

var set_ADpoint=function(x){
	for(var i=0;i<ad_img_src.length;i++){
		points[i].className="ad-point";
	}
	if(x<0||x>=ad_img_src.length)return;
	points[x].className="ad-point ad-point-active";
}
var set_ADimg=function(x){
	if(x<0||x>=ad_img_src.length)return;
	bigad.style.backgroundImage="url("+ad_img_src[x][0]+")";
	set_ADpoint(x);
	ad_link.href=ad_img_src[x][1];
	times=0;
}
var jumpAD=function(x){
	if(x<0||x>=ad_img_src.length)return;
	index=x;
	set_ADimg(x);
}
...
var resetPoint=function(){
	for(var i=0;i<ad_img_src.length;i++){
		...
		li.className="ad-point";
		// 设置data-index
		li.dataset.index=i;
		adpoints.appendChild(li);
	}
	points=document.getElementsByClassName('ad-point');
	// 设置点击事件，下表索引存在data-index的属性中
	for(var i=0;i<ad_img_src.length;i++){
...
var go_next_ADimg=function(){
	index++;
	if(index>=ad_img_src.length){
		index=0;
	}
	set_ADimg(index);
}
var go_last_ADimg=function(){
	index--;
	if(index<=-1){
		index=ad_img_src.length-1;
	}
	set_ADimg(index);
}
resetPoint();
set_ADimg(index);
//设置定时器，每100毫秒次数加一，加到50就是50*100毫秒即五秒执行一次切换动作
